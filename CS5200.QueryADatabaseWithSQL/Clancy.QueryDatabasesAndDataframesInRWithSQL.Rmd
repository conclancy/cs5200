...
```{r questionThree}
averages <- sqldf("SELECT item_count, avg(invoice_total) AS avg_invoice_cost FROM it GROUP BY item_count ORDER BY item_count" )

plot(
  averages$item_count, 
  averages$avg_invoice_cost,
  type = "b",
  xlab = "Invoice Items",
  ylab = "Invoice Total Cost", 
  main= "Average Invoice Cost by Item Count"
)
```
